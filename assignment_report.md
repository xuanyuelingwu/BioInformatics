...
2. 在 **Enter Query Sequence** 框中输入给定的蛋白序列：`MSTRSVSSSSYRRMFGGPGTASRPSSSRSYVTTSTRTYSLGSALRPSTSRSLYASSPGGVYATRSSAVRL`。
3. 在 **Choose Search Set** 区域，将 **Organism** 限定为 `Mus musculus (taxid:10090)`。
4. 展开 **Algorithm parameters**，将 **Max target sequences** 设置为 `10`，将 **Expect threshold**（E 值）设置为 `0.5`。
5. 点击 **BLAST** 按钮提交查询。

**操作过程截图**：
![BLASTP 参数设置](blastp_settings_full.webp)

**比对结果截图**：
![BLASTP 结果列表](blastp_traditional_results.webp)
![BLASTP 比对详情](blastp_alignments.webp)
...
**两者关系**：
E-value 和 P-value 之间存在数学关系：$P = 1 - e^{-E}$。当 E-value 很小（如 $E < 0.01$）时，P-value 约等于 E-value。在 BLAST 结果中，通常使用 E-value 来衡量比对的显著性，因为它更直观地反映了在特定数据库大小下的假阳性预期数量。
...
2. **局部匹配**：Smith-Waterman 算法找到的都是较短的局部匹配区域（如长度 50-60 左右），且包含较多 Gap（空位），Identity 仅在 40%-50% 左右。这进一步证明了打乱序列破坏了原有的结构域和保守基序。
...
1. **词语查找（Word Lookup / K-mer Hashing）**：
   BLAST 首先将查询序列分割成长度为 $W$ 的短片段（称为 Word 或 K-mer，蛋白质通常 $W=3$）。然后，它在数据库中快速扫描这些短片段的精确匹配或高分匹配（使用替换矩阵如 BLOSUM62 评估得分高于阈值 $T$ 的词）。这一步通过预先构建的**哈希表（Hash Table）**或有限状态自动机实现，查找速度极快。
2. **双击触发（Two-Hit Trigger）**：
   为了减少假阳性延伸，现代 BLAST 算法要求在同一条对角线上、距离在一定范围（$A$）内找到**两个**不重叠的 Word 匹配（Hits），才会触发后续的延伸步骤。
3. **无空位与有空位延伸（Ungapped and Gapped Extension）**：
   只有当找到符合条件的“种子”后，BLAST 才会向两端进行延伸。首先进行快速的无空位延伸，如果得分超过某个阈值，再进行耗时的有空位延伸（此时才局部使用动态规划）。
...
2. **对称的替换打分矩阵（Substitution Scoring Matrix, PAM）**：
   ...
   这是因为在比较两条现存物种的同源序列时，我们**无法确定进化的方向**（即不知道是 $i$ 突变成了 $j$，还是 $j$ 突变成了 $i$，或者它们都来自一个共同的祖先氨基酸）。
   为了解决这个问题，Dayhoff 等人在构建打分矩阵时，使用了**对数比值（Log-odds ratio）**：
   $$ S_{ij} = 10 \times \log_{10} \left( \frac{q_{ij}}{p_i p_j} \right) $$
   其中 $q_{ij}$ 是 $i$ 和 $j$ 发生替换的观测频率，$p_i$ 和 $p_j$ 是它们的背景频率。由于 $q_{ij} = q_{ji}$（在统计时将双向突变合并计算），且分母 $p_i p_j = p_j p_i$，因此计算出的打分矩阵必然是**对称**的。
...
[1] Wikipedia. "Point accepted mutation". https://en.wikipedia.org/wiki/Point_accepted_mutation
